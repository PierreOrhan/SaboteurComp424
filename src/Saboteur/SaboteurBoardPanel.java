...
public class SaboteurBoardPanel extends BoardPanel implements MouseListener, MouseMotionListener, ComponentListener {
    ...
    private static final int BOARD_DIM = SaboteurBoardState.BOARD_SIZE;
    ...
    final class GUIPiece {
        ...
        // Construct a piece!
        GUIPiece(Piece pieceType, int xPos, int yPos, SaboteurCard coord) {
    ...
    private SaboteurCard pieceSelection;
    ...
    private void processPlacePiece(MouseEvent e) {
        ...
        if (pbs.isPlaceLegal(dest)) {
            ...
            pieceSelection = new SaboteurCard(dest.getX(), dest.getY());
            pbs.getBoard()[dest.getX()][dest.getY()] = pbs.getTurnPlayer() == SaboteurBoardState.WHITE ? Piece.WHITE : Piece.BLACK;
            humanRepaint();
    ...
    private void completeMove(MouseEvent e) {
        ...
        SaboteurBoardState pbs = (SaboteurBoardState) getCurrentBoard().getBoardState();
        SaboteurMove move = new SaboteurMove(pieceSelection, quadSelection, secondQuad, pbs.getTurnPlayer());
        ...
        for (int i = 0; i < SaboteurBoardState.BOARD_SIZE; i++) {
            for (int j = 0; j < SaboteurBoardState.BOARD_SIZE; j++) {
                ...
                if (p != Piece.EMPTY) {
                    int xPos = j * SQUARE_SIZE + SQUARE_SIZE / 2;
                    int yPos = i * SQUARE_SIZE + SQUARE_SIZE / 2;
                    GUIPiece gp = new GUIPiece(p, xPos, yPos, new SaboteurCard(i, j));
                    boardPieces.add(gp);
                }
            }
        }
